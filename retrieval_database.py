...
def get_embed_model(encoder_model_name: str,
                    device: str = 'cpu',
                    retrieval_database_batch_size: int = 256) -> OpenAIEmbeddings:
    """
    Get embedding model.
    You can also code for other embedding models.
    
    :param encoder_model_name: Name of the encoder model. Options:
    ...
    :param retrieval_database_batch_size: Batch size.
    :return: The embedding model.
    """
    
    models = {
        'open-ai': OpenAIEmbeddings,
        'all-MiniLM-L6-v2': lambda: HuggingFaceEmbeddings(
            model_name=encoder_model_name,
            model_kwargs={'device': device},
            encode_kwargs={'device': device, 'batch_size': retrieval_database_batch_size}
        ),
        'bge-large-en-v1.5': lambda: HuggingFaceEmbeddings(
            model_name='BAAI/bge-large-en-v1.5',
            model_kwargs={'device': device},
            encode_kwargs={'device': device, 'batch_size': retrieval_database_batch_size}
        ),
        'e5-base-v2': lambda: HuggingFaceEmbeddings(
    ...
    if encoder_model_name in models:
        embed_model = models[encoder_model_name]()
    else:
        try:
...
def construct_retrieval_database(
    ...
    retrieval_database_batch_size: int = 256,
    chunk_size: int = 1500,
    ...
    :param data_name_list: The names of the datasets, placed in './Data/{data_name}'.
                           Optional: ['enron-mail', 'chatdoctor', 'wikitext-103'].
                           Also supports pre-processed datasets, e.g., 'enron-mail-body', 'enron-mail-strip'.
    :param split_method: The method to split the data, one per dataset or a single method for all.
    ...
    :param retrieval_database_batch_size: Batch size for querying the retrieval database.
    :param chunk_size: Chunk size for 'recursive_character' split method.
    :param chunk_overlap: Chunk overlap for 'recursive_character' split method.
    :return: A retrieval database of type langchain.vectorstores.chroma.Chroma.
    :class
    ...
    # Inner classes for text splitting
    ...
    # Helper function to get the appropriate splitter
    def get_splitter(method: str) -> TextSplitter:
        splitters = {
            'single_file': SingleFileSplitter(),
            'by_two_line_breaks': LineBreakTextSplitter(),
            'recursive_character': RecursiveCharacterTextSplitter(chunk_size=chunk_size, chunk_overlap=chunk_overlap)
        }
        return splitters.get(method)
    ...
    if split_method is None:
        ...
        split_method *= len(data_name_list)
    else:
        assert len(split_method) == len(data_name_list), "Mismatch in lengths of data_name_list and split_method"

    # Process datasets
    data_store_path = 'Data'
    all_split_texts = []
    for data_name, method in zip(data_name_list, split_method):
        data_path = os.path.join(data_store_path, data_name)
        documents = []
        ...
        print(f'File count for {data_name}: {len(documents)}')
        splitter = get_splitter(method)
        all_split_texts.extend(splitter.split_documents(documents))

    # Embed and construct the retrieval database
    embed_model = get_embed_model(encoder_model_name, device, retrieval_database_batch_size)
    retrieval_name = 'mix_' + '_'.join(data_name_list) if len(data_name_list) > 1 else data_name_list[0]
    vector_store_path = f"./RetrievalBase/{retrieval_name}/{encoder_model_name}"
    print(f'Generating Chroma database for {retrieval_name} using {encoder_model_name}')
    ...
    vectordb = None
    for chunk in chunk_list(all_split_texts, 41000):
