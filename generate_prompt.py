...
def get_question(question_prefix: List[str],
    ...
    def load_data(infor_name: str) -> List[str]:
        """Load data from a JSON file based on the type of information."""
        if 'Performance' in infor_name:
            data_name = infor_name.split('_')[1]
            file_path = f'Data/{data_name}-test/eval_input.json'
        else:
            file_path = f'Information/{infor_name}.json'
        
        with open(file_path, 'r', encoding='utf-8') as f:
            return json.load(f)
    
    questions = {}
    for i, prefix in enumerate(question_prefix or ['']):
        for j, suffix in enumerate(question_suffix or ['']):
            for k, adhesive in enumerate(question_adhesive or ['']):
                for l, infor_name in enumerate(question_infor):
                    data = load_data(infor_name)
                    question_list = [f"{prefix}{infor}{adhesive}{suffix}" for infor in data]
                    
                    key_suffix = '-'.join(filter(None, [str(i+1) if len(question_prefix) > 1 else '',
                                                        str(j+1) if len(question_suffix) > 1 else '',
                                                        str(k+1) if len(question_adhesive) > 1 else '',
                                                        str(l+1) if len(question_infor) > 1 else '']))
                    key = f"Q1-{key_suffix}" if key_suffix else "Q1"
                    questions[key] = question_list
                    
    return questions


def get_fewshot_questions(
    ...
    """
    Construct few-shot questions by concatenating examples with the question itself.

    :param question_prefix: List of prefixes for the question.
    :param question_suffix: List of suffixes for the question.
    :param question_adhesive: List of adhesives for the question.
    :param question_infor: List of information sources for the question.
    :param fewshot_examples: List of few-shot examples to prepend to each question.
    :return: Dictionary of few-shot questions generated for each combination.
    """
        ...
        for j, suffix in enumerate(question_suffix or ['']):
            for k, adhesive in enumerate(question_adhesive or ['']):
                for l, infor_name in enumerate(question_infor):
                    data = load_data(infor_name)
                    question_list = [
                        f"{' '.join(fewshot_examples)}{adhesive}{prefix}{infor}{adhesive}{suffix}" 
                        for infor in data
                    ]
                    ...
                    key_suffix = '-'.join(filter(None, [str(i+1) if len(question_prefix) > 1 else '',
                                                        ...
                                                        str(l+1) if len(question_infor) > 1 else '']))
                    key = f"Q1-{key_suffix}" if key_suffix else "Q1"
                    fewshot_questions[key] = question_list
...
def get_contexts(data_name_list: List[List[str]],
    ...
    summarize: List[Any],
    num_questions: int,
    questions_dic: Dict[str, List[str]],
    max_context_length: int = 2048
) -> Tuple[Dict[str, List[List[str]]], Dict[str, List[List[str]]], Dict[str, List[str]], Dict[str, Any]]:
    """
...
def get_prompt(settings: Dict, output_dir: str) -> List[str]:
    """
    Generate prompts based on the provided settings and save them to the specified directory.
    The prompt is consisted by:
        f'{suffix[0]}{united context}{template_adhesive}{suffix[1]}{question}{template_adhesive}{suffix[2]}'
     You can change the parameter at settings['template']
    :param settings: Dictionary containing all the settings for prompt generation.
    :param output_dir: The experiment name; all outputs will be saved under this directory.
    :return: A list of all the prompt storage paths with different parameters.
    """
    def save_json_files(output_path: str, question: List[str], prompt: List[str], source: List[str], context: List[str]) -> None:
        """
        Save the question, prompt, source, and context data to JSON files.
        
        :param output_path: The directory to save the JSON files.
        :param question: The list of questions.
        :param prompt: The list of generated prompts.
        :param source: The list of sources.
        :param context: The list of contexts.
        """
        with open(os.path.join(output_path, 'question.json'), 'w', encoding='utf-8') as file_q:
            json.dump(question, file_q)
        with open(os.path.join(output_path, 'prompts.json'), 'w', encoding='utf-8') as file_p:
            json.dump(prompt, file_p)
        with open(os.path.join(output_path, 'sources.json'), 'w', encoding='utf-8') as file_s:
            json.dump(source, file_s)
        with open(os.path.join(output_path, 'context.json'), 'w', encoding='utf-8') as file_c:
            json.dump(context, file_c)


    out_lst = []
    ques_set = settings['question']
    re_set = settings['retrival']
    tem_set = settings['template']

    questions = get_question(**ques_set)

    contexts, sources, questions, contexts_u = get_contexts(
    ...
    dir_ = [-1] * 2
    for i1, suffix in enumerate(tem_set['suffix']):
        ...
        for i2, adhesive in enumerate(tem_set['template_adhesive']):
            ...
            dir_suffix = 'T-' + '+'.join(t_dir)
            
            for key in contexts:
                context = contexts[key]
                context_u = contexts_u[key]
                source = sources[key]
                question_list = questions[key]

                output_subdir = f"{key}{dir_suffix}"
                output_path = os.path.join('Inputs&Outputs', output_dir, output_subdir)

                prompt = []
                if not isinstance(context_u, list):
                    # Summarize situation
                    prompt = []
                    os.makedirs(output_path, exist_ok=True)
                    with open(os.path.join(output_path, 'set.json'), 'w', encoding='utf-8') as file:
                        json.dump({
                            'suffix': suffix, 
                            'adhesive_prompt': adhesive, 
                            'adhesive_con': context_u[0], 
                            'infor': context_u[1]
                        }, file)
                else:
                    for i in range(len(question_list)):
                        prompt.append(f"{suffix[0]}{context_u[i]}{adhesive}{suffix[1]}{question_list[i]}{adhesive}{suffix[2]}")

                # Save results
                out_lst.append(output_subdir)
                os.makedirs(output_path, exist_ok=True)
                save_json_files(output_path, question_list, prompt, source, context)
