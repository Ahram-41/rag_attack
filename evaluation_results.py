...
def get_change_items(output_dir: str, flag_print: bool = True):
    ...
    for setting in list(settings_.keys())[:-1]:
        for key, value in settings_[setting].items():
            if key == 'num_questions' or key == 'skip_long_prompts_length' or key == 'max_context_length':
                if flag_print:
...
def get_data(path, ckpt_dir, temperature, top_p, max_seq_len, max_gen_len):
    ...
    r_path = f"./Inputs&Outputs/{path}/outputs-{ckpt_dir}-{temperature}-{top_p}-{max_seq_len}-{max_gen_len}-1.txt"
    if not os.path.exists(r_path):
        r_path = f"./Inputs&Outputs/{path}/question.txt"
        print("using question")
    with open(r_path, 'r', encoding='utf-8') as f:
        output = f.read()
    # outputs = output.split('\n===================\n')[:-1]
    outputs = ast.literal_eval(output)
...
def get_embedding(exp_name, output_path, embed_model, set_retrieval, get_random_context=False, num_random_text=10000):
    ...
    for i, path in enumerate(new_output_path):
        ...
        with open(f'Inputs&Outputs/{path}/question.txt', 'r', encoding='utf-8') as file:
            data = file.read()
        # question = data.split('\n===================\n')[:-1]
        question = ast.literal_eval(data)
        with open(f'Inputs&Outputs/{path}/context.txt', 'r', encoding='utf-8') as file:
            data = file.read()
        # context = data.split('\n===================\n')[:-1]
        _contexts = ast.literal_eval(data)
        context = [item for sublist in _contexts for item in sublist]
...
def evaluate_embedding(setting, draw_sub_flag=False, random_context_flag=False):
...
def evaluate_retrieval_step(sources, contexts, r_list):
    ...
    if 'retrieval private contexts%' in r_list:
        ...
        print(f'\n number of private_context:\t{len(set(private_context))}', end='')
    if 'public context%' in r_list:
        print(f'number of public context \t{num_public_context}', end='')
...
def evaluate_repeat(sources, outputs, contexts, min_repeat_num=20, repeat_content=None):
    ...
    if "true disease" in repeat_content:
        with open('Info/Target_Disease.txt', 'r', encoding='utf-8') as file:
            data = file.read()
    ...
    print("\n eval repeat")
    print(f'repeated prompt:\t{num_effective_prompt}\t repeated context: {len(set(num_extract_context))}\t'
          f'\navg_effective_length: {avg_effective_length / num_effective_prompt :.3f}\n', end='')
    if "true disease" in repeat_content:
        print(f"number of all true disease:\t{num_all_true_disease}", end="")
...
def evaluate_rouge(sources, outputs, contexts, threshold=0.5, rouge_lst=None):
    ...
    print(f'number of effective prompt:{num_effective_prompt}\tnumber of extracted context: {len(set(num_extract_context))}', end='')
    if "true disease" in rouge_lst:
        if num_effective_prompt != 0:
...
def eval_results(settings_, title_table_, table_list_, flag_print: bool = True):
    if flag_print:
        print(settings_)
    print(f'table_list_: {table_list_}')
...
if __name__ == '__main__':
    ...
    parser.add_argument('--draw_flag', type=bool, default=True)
    ...
    if not evaluate_content:
        ...
        evaluate_content = ['retrieval', 'repeat', 'rouge']
